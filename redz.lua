...
function WaitHRP(p524)
    if not p524 then return nil end
    local vhrp = p524.Character
    if not vhrp then return nil end
    return vhrp:WaitForChild("HumanoidRootPart", 10)
end
...
local vu523 = false
local vu590 = nil -- tween hiện tại

function topos(p540)
    local vu541 = game.Players.LocalPlayer
    local vu542 = vu541.Character
    local vu543 = vu542 and vu542:FindFirstChild("HumanoidRootPart")
    local vu544 = vu542 and vu542:FindFirstChildOfClass("Humanoid")

    if not vu542 or not vu543 or not vu544 or vu544.Health <= 0 then return end
    if not p540 then return end

    -- chuẩn hóa input
    local vu545
    local vu546 = typeof(p540)
    if vu546 == "CFrame" then
        vu545 = p540
    elseif vu546 == "Vector3" then
        vu545 = CFrame.new(p540)
    elseif vu546 == "Instance" and p540:IsA("BasePart") then
        vu545 = p540.CFrame
    else
        return
    end

    local vu547 = (vu545.Position - vu543.Position).Magnitude

    local vu548 = CheckNearestTeleporter(vu545)
    if vu548 then
        requestEntrance(vu548)
    end

    if not vu542:FindFirstChild("PartTele") then
        local vu549 = Instance.new("Part")
        vu549.Name = "PartTele"
        vu549.Size = Vector3.new(10,1,10)
        vu549.Anchored = true
        vu549.Transparency = 1
        vu549.CanCollide = true
        vu549.CFrame = vu543.CFrame
        vu549.Parent = vu542

        vu549:GetPropertyChangedSignal("CFrame"):Connect(function()
            if vu523 and vu542 and vu542:FindFirstChild("HumanoidRootPart") then
                task.wait()
                vu543.CFrame = vu549.CFrame
            end
        end)
    end
    ...
    if vu590 then
        pcall(function() vu590:Cancel() end)
        vu590 = nil
    end

    vu590 = game:GetService("TweenService"):Create(
        vu542.PartTele,
        TweenInfo.new(vu547 / 360, Enum.EasingStyle.Linear),
        { CFrame = vu545 }
    )

    vu590:Play()
    vu590.Completed:Connect(function(vu550)
        if vu550 == Enum.PlaybackState.Completed then
            if vu542:FindFirstChild("PartTele") then
                vu542.PartTele:Destroy()
            end
            vu523 = false
            vu590 = nil
        end
...
function stopTeleport()
    ...
    local vu552 = vu551.Character
    local vu553 = vu552 and vu552:FindFirstChild("HumanoidRootPart")
    ...
    if vu552 and vu552:FindFirstChild("PartTele") then
        pcall(function() vu552.PartTele:Destroy() end)
    end

    if vu553 and vu553:FindFirstChild("BodyClip") then
        pcall(function() vu553.BodyClip:Destroy() end)
    end
...
local vu560 = {}
function TP13(p561)
    local vu562 = game.Players.LocalPlayer.Character
    local vu563 = vu562 and vu562:FindFirstChild("HumanoidRootPart")
    if not vu563 then return end

    local vu564 = (p561.Position - vu563.Position).Magnitude
    local vu565 = game:GetService("TweenService"):Create(
        vu563,
        TweenInfo.new(vu564 / TweenSpeed, Enum.EasingStyle.Linear),
        { CFrame = p561 }
    )
    vu565:Play()

    function vu560.Stop()
        pcall(function() vu565:Cancel() end)
    end
    return vu560
end
...
local function vu1364()
    ...
    -- duyệt từng enemy và farm đến chết
    for _, mob in ipairs(list) do
        while true do
            ...
            if not mob or not mob.Parent or not hum or not mhrp or hum.Health <= 0 then
                break -- chuyển sang con tiếp theo
            end

            EquipWeapon(_G.SelectWeapon)
            topos(mhrp.CFrame * CFrame.new(0, 30, 0))
        end
    end
end
